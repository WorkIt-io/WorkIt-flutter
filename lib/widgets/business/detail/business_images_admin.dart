...
class BusinessImagesAdmin extends ConsumerStatefulWidget {
  const BusinessImagesAdmin({super.key});

  @override
  ConsumerState<BusinessImagesAdmin> createState() => _BusinessImagesState();
}

class _BusinessImagesState extends ConsumerState<BusinessImagesAdmin> {
  ...
  Future<void> onUploadImagePress(
    ...
    if (imageFile != null) {
      try {
        await ref
            .read(imageBusinessNotifierProvider.notifier)
  ...
  Future<void> onLastDeleteImagePress() async {
    await ref.read(imageBusinessNotifierProvider.notifier).removeLastImage();
